...
2. Telegram message links, for example: `https://t.me/acherkrau/1097`. **Even if the channel prohibits forwarding and saving, the bot can still download its files.** When sending multiple Telegram message links in one message, add `group:merge` or use `/merge`; the bot merges parsed photos/videos into albums in input order. Files that cannot join an album are skipped instead of being sent as standalone messages.
