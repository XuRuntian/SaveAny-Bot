...
2. Telegram 消息链接, 例如: `https://t.me/acherkrau/1097`. **即使频道禁止了转发和保存, Bot 依然可以下载其文件.** 同一条消息中发送多条 Telegram 消息链接时, 可以加一行 `group:merge` 或使用 `/merge`, Bot 会将解析出的图片/视频按输入顺序合并为相册批量保存. 无法加入相册的文件会被跳过, 不会降级为单条消息.
